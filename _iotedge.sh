...
iotedge::RestartIoTEdge()
{
	  ...
	  if [ "$status" != "active" ]; then
		echo "IoTEdge failed to start."
		echo "Please try to restat the service manually running 'sudo systemctl restart iotedge'."
		return 1
	  fi
	  echo $dependency_errmssg
	  echo "IoTEdge Runtime Ready."
	  return 0
}
...
iotedge::SetIoTEdgeDaemonForProxy()
{
...
iotedge::SetDockerProxySetttings()
{
	envString="Environment"
	local proxy_url="$(iotedge::GetProxyUrl)"
